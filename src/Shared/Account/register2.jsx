...
const Register = () => {
  ...
  const handlesubmit = (e) => {
    ...
    axios
      .post('https://blood-donation-backend-production.up.railway.app/user/register', intialValues)
      .then((res) => {
        if (res) {
    ...
    uploadBytes(imageRef, image).then((snapshot) => {
      getDownloadURL(snapshot.ref).then((url) => {
        if (url) {
          setIsLoading(true);
          axios
            .put('https://blood-donation-backend-production.up.railway.app/user/addImage/', {
              _id: userId,
              imageUrl: url
            })
            .then((res) => {
    ...
    <div className="max-w-4xl md:mx-auto mx-2 bg-white shadow-2xl border rounded-lg h-auto py-5 mt-10">
      ...
      <div className="mx-2 pt-5 md:mx-2">
        {regStep === 1 ? (
          ...
          <div className="mx-2 pt-5 md:mx-2">
            <form className="w-full p-6 mx-auto" onSubmit={uploadImage}>
              <div>
                <label className="text-sm tracking-wide mb-2">
                  ছবি আপলোড করুন <span className="text-red-600">*</span> <small> </small>
                </label>
                <input
                  className="appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="file"
